...
| ラベル | 意味 |
|--------|------|
| `[観測値：単一環境・単一運用者・N未公開]` | 著者の運用環境で計測。公開ベンチマークではない。サンプル数は非公開 |
| `[設計目標：公開ベンチマークではない]` | 設計上の目標値。実測・独立再現されていない |
| `[例示値：実測ではない]` | 概念理解のための説明シナリオ。データではない |
| `[有料版で詳細開示：公開版は要約のみ]` | 詳細な実装・計測は公開版では非公開 |

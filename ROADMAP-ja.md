# ロードマップ

本プロジェクトはアクティブに維持されています。以下が現在の方向性です。

## 現在の公開リポジトリ

> Phase 1 / Phase 2 は将来の公開フェーズであり、価格帯ではありません。「現在」に書かれている内容は、すべてこのリポジトリで無料公開されています。

- Claude、GPT、Copilotにまたがる40の失敗モード分類
- Control OSテンプレート（コピペ対応）
- 15件の成果ページ（6件詳細 + 9件概要）
- インタラクティブデモ（GitHub Pages）
- 方法論を説明するフレームワーク文書

## 次: Phase 1

- 10件の詳細設計書
- FM 60項目のより深い分類
- 図、サンプル会話、reason_codeテーブル
- Level B成果の定量データ

リリース時期は未定です。本リポジトリをWatchして更新を確認してください。

## 将来: Phase 2

- 全132失敗モードの完全詳細
- 検証ログとソース抜粋の完全版
- トークン効率化のBefore/After実測データ

## 将来: 書籍

- SHI理論によるAI統治の全7章
- 再現手順（具体的な鍵は一切なし）

---

## ロードマップへの影響の与え方

Issueから寄せられたコミュニティ観測はFM分類に反映される可能性があります。貢献者はCHANGELOG-ja.mdにクレジットされます。

- [観測レポートを提出](https://github.com/naoyukioyama561-alt/SHI-Claude-Control-OS/issues/new?template=observation-report.md) — あなたの環境で観測したことを共有
- [主張への挑戦](https://github.com/naoyukioyama561-alt/SHI-Claude-Control-OS/issues/new?template=claim-challenge.md) — エビデンスの改善に貢献
- [Issueを開く](https://github.com/naoyukioyama561-alt/SHI-Claude-Control-OS/issues) — 質問、アイデア提案、方法論に関する議論を始める

> [README-ja.md](README-ja.md) に戻る

...
AIコーディングアシスタントの6つの構造的問題に対する品質管理システム：
1. 確認を飛ばす
2. 読むべきものを読まない
3. 省略する
4. 1件1件が雑
...
既存システム（assist/verify、cc/directives、behavior/pattern、incident_log、learning）の上に構築。
...
| 層 | 責務 | 既存接続先 |
|---|---|---|
| 第0層（観測ゲート） | 実行前に観測・目標定義・承認を確認 | /cc/directives/check-response |
| 第1層（自分） | 自己検知+即時修正 | audit_log自己記録 |
| 第2層（構造） | hook/watcher自動検知+差し戻し | hook、watcher_ai、パターン検知 |
| 第3層（完了定義） | エビデンス+5セット検査 | /local-ai/assist/verify（B軸） |
| 第4層（第三者検証） | パターン昇格+incident化+学習制御 | cc_relay、incident_log、/behavior/pattern/add、/learning/* |
...
| DISCREPANCY_DETECTED | cc_relayとの不一致 | 第4層 |
...
| 条件 | アクション |
|---|---|
| 同一reason_codeが2回以上 | /behavior/pattern/addにパターン登録 |
| 省略系でユーザー反論が発生 | incident_logに記録+パターン登録 |
...
| 条件 | 学習データカテゴリ |
|---|---|
| /assist/verify合格+behavior_check合格+5セット完備 | 高品質データ（優先蓄積） |
...
| #2 読み飛ばし | ファイルを読んだか | cc_context Read記録+引用照合 |
| #3 省略 | 全件やったか | items_count照合+check-response |
| #4 雑な作業 | 設計通りか | /assist/verify B軸5項目 |

...
> This document shows the **structural overview** of the quality management system.
...
A quality management system addressing 6 structural problems in AI coding assistants:
...
Built on top of existing subsystems (assist/verify, cc/directives, behavior/pattern, incident_log, learning).
...
|-------|---------------|---------------------|
| Layer 0 (Observation Gate) | Pre-execution: observe, define target, confirm approval | /cc/directives/check-response |
| Layer 1 (Self) | Self-detection + immediate correction | audit_log self-recording |
| Layer 2 (Structure) | Hook/watcher auto-detection + rollback | hooks, watcher_ai, pattern detection |
| Layer 3 (Completion Definition) | Evidence + 5-set inspection | /local-ai/assist/verify (B-axis) |
| Layer 4 (Third-party Verification) | Pattern escalation + incident logging + learning control | cc_relay, incident_log, /behavior/pattern/add, /learning/* |
...
| DISCREPANCY_DETECTED | Mismatch with cc_relay | Layer 4 |
...
| Condition | Action |
|-----------|--------|
| Same reason_code occurs 2+ times | Register as pattern in /behavior/pattern/add |
| Omission causes user pushback | Record in incident_log + register pattern |
...
|-----------|----------------------|
| assist/verify pass + behavior_check pass + 5-set complete | High-quality data (priority accumulation) |
...
| #2 Read what needs reading | Did you read the file? | cc_context Read record + quote matching |
| #3 No omissions | Did you do all items? | items_count matching + check-response |
| #4 Careful work | Does it match the design? | /assist/verify B-axis 5 items |

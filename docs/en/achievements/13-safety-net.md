# Achievement No.13: Self-Restraint & Safety Net (Block Hooks + Deny Rules)
...
- **Write scope limitation**: The current CC can only write to cc_dialogue.md — all other write operations are blocked
- **dialogue_blocker_hook**: Additional hook specifically preventing unauthorized dialogue file modifications
...
- The permission separation (write only cc_dialogue.md, deny all destructive commands) creates a safe sandbox where the AI can operate freely within strict boundaries
...
| Image | Description |
|-------|-------------|
| ![Permission separation](../../images/13-safety-net/IMG_074.png) | Permission separation complete (write: cc_dialogue.md only, deny: rm, mv, cp, etc.) |
...
> This is a **paid-tier achievement** (Phase1). The safety design principles are shared here. For hook implementations, deny rule configurations, and supervisor process details, see the paid tiers.
>
> Phase1 provides the safety architecture overview. Phase2 provides watcher_infra code excerpts and block hook implementations. The book includes the complete safety system with construction procedures.

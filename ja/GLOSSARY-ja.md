...
| 公開リポジトリ vs 非公開参考資料 | 公開リポジトリ=このGitHubリポジトリで利用可能なすべて。非公開参考資料=このリポジトリの公開評価対象外に意図的に置かれた資料 |

---

## モデル名に関する注記

本リポジトリのモデル名（Claude Opus 4.6, Claude Sonnet 4.6, GPT-5）は観測期間（2025年末〜2026年3月）に利用可能だったバージョンを指します。現在の正式なモデル名は異なる場合があります。本リポジトリの数値を引用する際は、必ず観測期間のコンテキストを含めてください。
